...
// Mock helmet middleware to avoid external package dependency
const helmet = (_options?: any) => (req: Request, res: Response, next: NextFunction) => {
...
Reply with ONLY valid JSON, no explanation.`;
  ...
  if (process.env.NODE_ENV !== "production") {
    let webPath = path.resolve(process.cwd(), "../web");
    if (!fs.existsSync(path.join(webPath, "package.json"))) {
      webPath = path.resolve(process.cwd(), "apps/web");
    }
    ...
    let distPath = path.resolve(process.cwd(), "../web/dist");
    if (!fs.existsSync(distPath)) {
      distPath = path.resolve(process.cwd(), "apps/web/dist");
    }
